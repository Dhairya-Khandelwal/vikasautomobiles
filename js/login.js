...
function updateLoginProgress() {
  ...
  if (isPasswordFormVisible) {
    ...
    progress = 80;
    const otpVal = document.getElementById('login-otp-value')?.value.trim() || "";
    if (otpVal) {
      progress += 20;
    }
  }
...
function initLoginPage() {
  ...
  if (otpInput) {
    otpInput.addEventListener('input', updateLoginProgress);
  }
...
async function handlePasswordLogin(event) {
  ...
  try {
    ...
    setTimeout(() => {
      // 2. Generate secure mock 6-digit OTP code for 2FA verification step
      currentGeneratedOtp = Math.floor(100000 + Math.random() * 900000).toString();

      // Log OTP generation
      window.API.addOtpLog(user.mobile, user.email, currentGeneratedOtp, "sent");

      // 3. Update the OTP Form UI
      const maskedMobile = user.mobile.length >= 10 
      ...
      if (debugHintEl) {
        debugHintEl.innerText = `DEVELOPMENT OTP CODE: ${currentGeneratedOtp} (Use this code to verify)`;
      ...
      window.UTILS.showToast(`OTP verification code generated.`, "success");
...
async function handleOtpVerification(event) {
  ...
  if (!enteredOtp) {
    window.UTILS.showToast("Please enter the verification code.", "error");
    return;
  }

  if (enteredOtp !== currentGeneratedOtp) {
    if (verifiedUser) {
      window.API.addOtpLog(verifiedUser.mobile, verifiedUser.email, enteredOtp, "failed");
    }
    window.UTILS.showToast("Invalid OTP code. Please enter the correct verification code.", "error");
    return;
  }
  ...
  window.API.updateOtpLogStatus(verifiedUser.email, "verified");
...
let otpResendCooldownInterval = null;
let otpResendSecondsRemaining = 0;
...
function startOtpResendCooldown() {
  ...
  btn.innerText = `Resend in ${otpResendSecondsRemaining}s`;

  if (otpResendCooldownInterval) {
...
async function handleResendOtp() {
  if (otpResendSecondsRemaining > 0) return;
  if (!verifiedUser) {
    window.UTILS.showToast("Verification session expired. Please log in again.", "error");
    cancelOtpFlow();
    return;
  ...
  window.UTILS.showLoader("Generating new OTP...");
  try {
    // Generate secure mock 6-digit OTP code
    currentGeneratedOtp = Math.floor(100000 + Math.random() * 900000).toString();

    // Log OTP generation
    window.API.addOtpLog(verifiedUser.mobile, verifiedUser.email, currentGeneratedOtp, "sent");
    ...
    updateLoginProgress();
    window.UTILS.showToast("A new OTP verification code has been generated.", "success");
    startOtpResendCooldown();
  } catch (err) {
    window.UTILS.showToast("Failed to regenerate OTP. Please try again.", "error");
  } finally {
    window.UTILS.hideLoader();
  }
}
