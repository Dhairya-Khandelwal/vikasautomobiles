/* Retailer Dashboard Controller - Vikas Automobiles */

let currentRetailerSession = null;
let productsList = [];
...
async function loadRetailerDashboard() {
  window.UTILS.showLoader("Syncing wallet ledger...");
  try {
    ...
    document.getElementById("nav-points-balance").innerText = (currentRetailerSession.points || 0) + " Pts";
    document.getElementById("dash-points-approved").innerText = currentRetailerSession.points || 0;
    ...
    renderMilestoneRewards(currentRetailerSession.points || 0);

    // Load active announcements
    await window.NOTIFICATIONS.renderDashboardAnnouncements("partner-notices-feed", "retailer");

  } catch (e) {
    console.error("Retailer dashboard refresh error", e);
    window.UTILS.showToast("Failed to refresh dashboard stats.", "error");
  } finally {
    window.UTILS.hideLoader();
  }
}

// 2. PARTS CATALOG FOR RETAILER
    ...
    <div class="bg-slate-50 p-4 rounded-xl border border-slate-200 hover:border-blue-500/30 hover:shadow transition flex flex-col justify-between space-y-4">
      ...
      <div class="p-2.5 bg-slate-100/50 rounded-lg border border-slate-200 space-y-1 font-mono text-[10px]">
        <div class="flex justify-between">
          <span class="text-slate-500">Retailer Price:</span>
          <span class="font-bold text-slate-800">${window.UTILS.formatCurrency(p.retailerPrice)}</span>
        </div>
        <div class="flex justify-between">
          <span class="text-slate-500">Retailer Reward:</span>
          <span class="font-black text-blue-600">+${p.retailerPoints} Pts</span>
        </div>
      </div>
      ...
      <div class="flex items-center justify-between">
        ...
        <span class="text-[9px] font-mono ${p.stock <= p.minStock ? 'text-red-600 font-bold' : 'text-emerald-600 font-bold'}">${p.stock > 0 ? 'In Stock' : 'Out of Stock'}</span>
      </div>
    </div>
  `).join("");
}
...
// 3. SCAN & EARN QR SIMULATOR
function saveToRecentScans(productId) {
  if (!productId) return;
  const email = (currentRetailerSession || {}).email || "unknown";
    ...
    html += `
      <div onclick="selectRecentScan('${prod.id}')" class="group flex items-center justify-between p-2 bg-slate-50 border border-slate-100 hover:border-blue-200 hover:bg-blue-50/50 rounded-xl cursor-pointer transition">
        ...
        <div class="text-right">
          <span class="text-[10px] font-mono font-bold text-blue-600">+${prod.retailerPoints} Pts</span>
        </div>
      </div>
    `;
  });
...
async function loadScannerForm() {
  try {
    ...
    select.innerHTML = '<option value="">-- Choose Product Sticker --</option>' + 
      products.map(p => `<option value="${p.id}">${p.id} - ${p.name} (${p.retailerPoints} Pts)</option>`).join("");

    renderRecentScans();
  } catch (e) {
    console.error("Error loading products for retailer scan simulator", e);
  }
}
...
async function calculateMockScanPoints() {
  ...
  // Multiply by setting modifiers
  const policies = await window.API.getSettings();
  const basePoints = prod.retailerPoints * qty;
  const total = Math.round(basePoints * (policies.retailerMultiplier || 1.0));
  ...
  if (infoCard) {
    ...
    document.getElementById("scan-info-points").innerText = `${prod.retailerPoints} Pts each (Multiplier: x${policies.retailerMultiplier || 1.0})`;
...
async function submitManualLoyaltyScanClaim() {
  ...
  try {
    ...
    const policies = await window.API.getSettings();
    const pointsCalculated = Math.round((prod.retailerPoints * qty) * (policies.retailerMultiplier || 1.0));
    ...
    const claim = {
      email: currentRetailerSession.email,
      fullname: currentRetailerSession.fullname,
      role: currentRetailerSession.role,
      firmName: currentRetailerSession.firmName,
      productID: prod.id,
...
function calculateRedemptionRequiredPoints(value) {
  ...
  if (balInput) balInput.innerText = `${currentRetailerSession.points || 0} Pts`;
}
...
async function handleRedemptionSubmit(e) {
  ...
  if (!selectedReward) {
    window.UTILS.showToast("Please choose a gift reward option.", "warning");
    return;
  }

  const reqPts = selectedReward.pointsRequired;
  if ((currentRetailerSession.points || 0) < reqPts) {
    window.UTILS.showToast(`Insufficient wallet balance! Requires ${reqPts} Pts, you have ${currentRetailerSession.points || 0} Pts.`, "error");
    return;
  }
  ...
  window.UTILS.showLoader("Transmitting gift redemption request...");
  try {
    await window.API.submitRedemption(
      currentRetailerSession.email,
      currentRetailerSession.fullname,
      currentRetailerSession.role,
      currentRetailerSession.firmName || "",
      selectedReward.id,
      selectedReward.name,
      reqPts
    );
...
// 6. HELPLINE SUPPORT CHAT BOT SIMULATOR
function handleSendSupportMessage(e) {
  ...
  userMsg.innerHTML = `
    <span class="text-[9px] text-[#0B192C]/70 block font-bold font-sans mb-1 uppercase">You (${currentRetailerSession.fullname})</span>
    ${text}
  `;
  ...
  // Bot response triggers in 1 second
  setTimeout(() => {
    ...
    let botReply = "Bhai, your scan is saved. Central approval takes 2-4 hours. Keep scanning, and your Paytm cash will transfer instantly!";
    if (text.toLowerCase().includes("paytm") || text.toLowerCase().includes("gpay") || text.toLowerCase().includes("cash")) {
      botReply = "Bhai! Paytm / PhonePe cashback cash-out is instant. Just select the reward, enter your correct UPI ID (e.g. name@paytm), and confirm.";
    } else if (text.toLowerCase().includes("scanned") || text.toLowerCase().includes("scan") || text.toLowerCase().includes("points")) {
      botReply = "Bhai, if you have scanned the box code but points are pending, Suresh (Central Warehouse Admin) will audit and approve it within 2 hours.";
    }
...
async function initRetailerPage() {
  window.UTILS.showLoader("Opening Retailer Portal workspace...");
  try {
    currentRetailerSession = window.AUTH.verifySession(["retailer"]);
    if (!currentRetailerSession) return;

    // Load sidebar details
    document.getElementById("sidebar-username").innerText = currentRetailerSession.fullname;
    if (currentRetailerSession.photo) {
      document.getElementById("sidebar-avatar").src = currentRetailerSession.photo;
    }

    // Set dynamic custom tier badge based on accumulated points
    const points = currentRetailerSession.points || 0;
    let tierText = "BRONZE MEMBER";
    let tierClass = "text-slate-500 font-mono";

    if (points >= 15000) {
      tierText = "GOLD PLAN PARTNER";
      tierClass = "text-amber-400 font-mono font-bold tracking-widest";
    } else if (points >= 5000) {
      tierText = "SILVER VALUE PARTNER";
      tierClass = "text-blue-400 font-mono font-bold tracking-widest";
    }
    ...
    console.error("Retailer view failed to launch", e);
    window.UTILS.showToast("Session verification failed.", "error");
  } finally {
    window.UTILS.hideLoader();
  }
}
...
window.initRetailerPage = initRetailerPage;
...
function openProfileSettingsModal() {
  ...
  document.getElementById("profile-modal-email").value = session.email || "";
...
async function claimMilestoneReward(name, pointsRequired) {
  if ((currentRetailerSession.points || 0) < pointsRequired) {
  ...
  try {
    // Deduct points
    await window.API.updateUserPoints(currentRetailerSession.email, -pointsRequired);
    ...
    // Log system activity
    await window.API.logAction(currentRetailerSession.fullname, "Redeemed Milestone Reward", "retailer", `User redeemed ${pointsRequired} pts for ${name}`);
  } catch (ex) {
    window.UTILS.showToast(ex.message, "error");
  } finally {
    window.UTILS.hideLoader();
