  ...
  tbody.innerHTML = filtered.map(c => {
    ...
    const actions = c.status === "pending"
      ? `<button onclick="processOwnerPurchaseClaim('${c.id}', 'approved')" class="px-2 py-1 bg-emerald-600 hover:bg-emerald-500 text-[#0B192C] font-black rounded text-[9px] uppercase transition mr-1 cursor-pointer">Approve</button>
         <button onclick="processOwnerPurchaseClaim('${c.id}', 'rejected')" class="px-2 py-1 bg-red-650 hover:bg-red-600 text-white font-black rounded text-[9px] uppercase transition cursor-pointer">Reject</button>`
      : `<span class="text-[10px] text-slate-500 italic font-mono uppercase font-semibold">Processed</span>`;
