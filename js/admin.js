...
async function loadDashboardMetrics() {
  ...
  try {
    ...
    const lowStockEl = document.getElementById("stat-stock-low");
    if (lowStockEl) lowStockEl.innerText = lowStockSKUs;
    ...
    const alertBody = document.getElementById("quick-low-stock-tbody");
    if (alertBody) {
      const lowStockItems = products.filter(p => (p.stock || 0) <= (p.minStock || 5));
      if (lowStockItems.length === 0) {
        alertBody.innerHTML = `
          <tr>
            <td colspan="3" class="py-4 text-center text-slate-400 font-mono text-[11px]">No stock alerts.</td>
          </tr>
        `;
      } else {
        alertBody.innerHTML = lowStockItems.map(p => `
          <tr class="border-b border-slate-100 text-[11px]">
            <td class="py-2.5 font-bold text-slate-800">${p.name}<br><span class="text-[9px] text-slate-400 font-mono">${p.id}</span></td>
            <td class="py-2.5 text-rose-600 font-mono font-bold">${p.stock || 0}</td>
            <td class="py-2.5 text-slate-500 font-mono">${p.minStock || 5}</td>
          </tr>
        `).join("");
      }
    }
...
function renderProductsListTable(list) {
  ...
  if (list.length === 0) {
    tbody.innerHTML = `
      <tr>
        <td colspan="10" class="py-6 text-center text-slate-500 font-mono">No products in database. Create one below.</td>
      </tr>
    `;
    return;
  }
  ...
  tbody.innerHTML = list.map(p => {
    ...
    return `
      <tr class="border-b border-slate-100 text-xs hover:bg-slate-50 transition">
        ...
        <td class="py-3 font-mono font-bold ${p.stock <= (p.minStock || 5) ? 'text-red-600' : 'text-slate-800'}">${p.stock}</td>
        <td class="py-3">
...
function handleProductFormSubmit(e) {
  ...
  const fields = [
    ...
    { from: "prod-stock", to: "prod-stock" },
    { from: "prod-minstock", to: "prod-minstock" },
  ...
  try {
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      const product = {
        ...
        stock: Number(cols[9]) || 50,
        minStock: Number(cols[10]) || 10
      };
  ...
  tbody.innerHTML = list.map(c => {
    ...
    const actions = isPending
      ? `<button onclick="processLoyaltyClaim('${c.id}', 'approved')" class="px-2 py-1 bg-emerald-600 hover:bg-emerald-500 text-white font-black text-[9px] rounded uppercase transition mr-1 cursor-pointer">Verify</button>
         <button onclick="processLoyaltyClaim('${c.id}', 'rejected')" class="px-2 py-1 bg-red-650 hover:bg-red-600 text-white font-black text-[9px] rounded uppercase transition cursor-pointer">Reject</button>`
      : `<span class="text-slate-500 font-mono text-[9px] uppercase font-bold">Processed</span>`;
    ...
    return `
      <tr class="border-b border-slate-100 text-xs hover:bg-slate-50 transition">
        ...
        <td class="py-3 font-sans">
          ...
          <span class="text-[10px] text-slate-500 font-mono">${c.email} (${c.role.toUpperCase()})</span>
        </td>
  ...
  tbody.innerHTML = list.map(u => {
    ...
    return `
      <tr class="border-b border-slate-100 text-xs hover:bg-slate-50 transition">
        ...
        <td class="py-3 font-bold text-slate-900">${u.fullname}<br><span class="text-[9px] text-slate-500 font-mono font-normal">${u.email}</span></td>
...
async function loadInventory() {
  try {
    ...
    if (products.length === 0) {
      tbody.innerHTML = `
        <tr>
          <td colspan="7" class="py-6 text-center text-slate-500 font-mono">No stock records in the ledger.</td>
        </tr>
      `;
      return;
    }
    ...
    tbody.innerHTML = products.map(p => {
      ...
      const statusBadge = isLow 
        ? `<span class="px-2 py-0.5 bg-red-100 text-red-700 border border-red-200 text-[9px] font-bold rounded uppercase font-mono">LOW STOCK</span>`
        : `<span class="px-2 py-0.5 bg-emerald-100 text-emerald-700 border border-emerald-200 text-[9px] font-bold rounded uppercase font-mono">ADEQUATE</span>`;

      return `
        <tr class="border-b border-slate-100 text-xs hover:bg-slate-50 transition">
          <td class="py-3 font-mono font-bold text-slate-500">${p.id}</td>
          <td class="py-3 font-bold text-slate-800">${p.name}</td>
          <td class="py-3 text-slate-600 font-semibold">${p.brand}</td>
          <td class="py-3 font-mono font-extrabold ${isLow ? 'text-red-600' : 'text-slate-800'}">${p.stock}</td>
          <td class="py-3 font-mono text-slate-500">${p.minStock}</td>
          <td class="py-3">${statusBadge}</td>
          <td class="py-3 text-right">
            <button onclick="quickRefillStock('${p.id}')" class="px-2.5 py-1 bg-blue-600 hover:bg-blue-500 text-white font-black text-[9px] rounded uppercase transition">Refill +50</button>
          </td>
        </tr>
      `;
    }).join("");
    ...
    // Sync the header safety count indicator badge if present
...
function loadProductQrMetadata() {
  ...
  if (prod) {
    ...
    if (qtyField) qtyField.value = prod.stock || "1";
  }
}
...
function openLoyaltyQrModal(userEmail) {
  ...
  try {
    ...
    document.getElementById("qr-card-email").innerText = `Email: ${user.email}`;
...
async function openEditPartnerModal(email) {
  try {
    ...
    document.getElementById("edit-partner-email").value = user.email || "";
  ...
  tbody.innerHTML = list.map(rdm => {
    ...
    const actionsHtml = isPending
      ? `<button onclick="processRedemption('${rdm.id}', 'approved')" class="px-2 py-1 bg-emerald-600 hover:bg-emerald-500 text-white font-black text-[9px] rounded uppercase transition mr-1 cursor-pointer">Ship</button>
         <button onclick="processRedemption('${rdm.id}', 'rejected')" class="px-2 py-1 bg-rose-650 hover:bg-rose-600 text-white font-black text-[9px] rounded uppercase transition cursor-pointer">Cancel</button>`
      : `<span class="text-slate-400 font-mono text-[9px] uppercase font-bold">Completed</span>`;
    ...
    return `
      <tr class="border-b border-slate-100 text-xs hover:bg-slate-50 transition">
        <td class="py-3 font-sans">
          ...
          <span class="text-[10px] text-slate-500 font-mono">${rdm.email} (${rdm.role.toUpperCase()})</span>
        </td>
